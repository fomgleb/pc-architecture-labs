...
int main() {
	...
	/* No 1 */
	error_code = read_drive(1, 0, 35, 1, buf); /* 1 0 33 */
	for (i = 0; i < 512; i += 4) {
		printf("%0.2x%0.2x%0.2x%0.2x;;", buf[i+3], buf[i+2], buf[i+1], buf[i]);
	}
	...
	printf("\nMY FILE:\n");
	read_drive(28, 2, 24, 1, files);
	print_file(files[11]);

	/* No 3 */
	printf("\nClusters path:\n");
	current_cluster = files[11].initial_cluster2_part;
	while(current_cluster != 0xFFFF) {
		printf("%0.4x -> ", current_cluster);
		read_drive(1, 0, 33 + current_cluster / 128, 1, buf);
		current_cluster = (buf[(current_cluster % 128) * 4 + 1] << 8) | (buf[(current_cluster % 128) * 4]);
	}
	printf("%x(end)", current_cluster);
